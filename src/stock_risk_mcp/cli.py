...
def build_command_parser() -> argparse.ArgumentParser:
    ...
    def add_kiwoom_oauth_args(command, *, include_force_refresh: bool = False) -> None:
        command.add_argument("--kiwoom-environment", default="MOCK", choices=["MOCK", "REAL"])
        credential_group = command.add_mutually_exclusive_group(required=True)
        credential_group.add_argument("--credential-ref")
        credential_group.add_argument("--appkey-ref-path")
        command.add_argument("--secretkey-ref-path")
        command.add_argument("--token-store-root", default="local_data/kiwoom_oauth_tokens")
        command.add_argument("--allow-real-chart-capture", action="store_true")
        ...
        command.add_argument("--acknowledge-credential-redaction", action="store_true")
        command.add_argument("--output-file", type=Path)
        if include_force_refresh:
            command.add_argument("--force-refresh-token", action="store_true")

    legacy_evaluate = subparsers.add_parser(
        "legacy-evaluate",
        help="Compatibility route for the legacy local risk MVP proposal evaluator.",
        description="Evaluate a stock trade proposal with the local risk MVP.",
    )
    add_proposal_args(legacy_evaluate)
    ...
    add_kiwoom_oauth_args(kiwoom_oauth_token_preflight)
    kiwoom_oauth_token_issue = subparsers.add_parser("kiwoom-oauth-token-issue-run")
    add_kiwoom_oauth_args(kiwoom_oauth_token_issue, include_force_refresh=True)
    ...
    kiwoom_rest_rank_adapter_check = subparsers.add_parser("kiwoom-rest-rank-adapter-check")
    kiwoom_rest_rank_adapter_check.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_rank_adapter_check.add_argument("--output-file", type=Path)
    kiwoom_rest_rank_request_report = subparsers.add_parser("kiwoom-rest-rank-request-report")
    kiwoom_rest_rank_request_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_rank_request_report.add_argument("--output-file", type=Path)
    kiwoom_rest_rank_mocked_response_report = subparsers.add_parser("kiwoom-rest-rank-mocked-response-report")
    kiwoom_rest_rank_mocked_response_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_rank_mocked_response_report.add_argument("--output-file", type=Path)
    kiwoom_rest_canonical_rank_report = subparsers.add_parser("kiwoom-rest-canonical-rank-report")
    kiwoom_rest_canonical_rank_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_canonical_rank_report.add_argument("--output-file", type=Path)
    kiwoom_rest_canonical_outlier_report = subparsers.add_parser("kiwoom-rest-canonical-outlier-report")
    kiwoom_rest_canonical_outlier_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_canonical_outlier_report.add_argument("--output-file", type=Path)
    kiwoom_rest_rank_continuation_report = subparsers.add_parser("kiwoom-rest-rank-continuation-report")
    kiwoom_rest_rank_continuation_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_rank_continuation_report.add_argument("--output-file", type=Path)
    kiwoom_rest_rank_readonly_safety_report = subparsers.add_parser("kiwoom-rest-rank-readonly-safety-report")
    kiwoom_rest_rank_readonly_safety_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_rank_readonly_safety_report.add_argument("--output-file", type=Path)
    kiwoom_rest_rank_v7_integration_report = subparsers.add_parser("kiwoom-rest-rank-v7-integration-report")
    kiwoom_rest_rank_v7_integration_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_rank_v7_integration_report.add_argument("--output-file", type=Path)
    kiwoom_rest_rank_gap_report = subparsers.add_parser("kiwoom-rest-rank-gap-report")
    kiwoom_rest_rank_gap_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_rank_gap_report.add_argument("--output-file", type=Path)
    kiwoom_rest_quote_adapter_check = subparsers.add_parser("kiwoom-rest-quote-adapter-check")
    kiwoom_rest_quote_adapter_check.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_quote_adapter_check.add_argument("--output-file", type=Path)
    kiwoom_rest_quote_request_report = subparsers.add_parser("kiwoom-rest-quote-request-report")
    kiwoom_rest_quote_request_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_quote_request_report.add_argument("--output-file", type=Path)
    kiwoom_rest_quote_mocked_response_report = subparsers.add_parser("kiwoom-rest-quote-mocked-response-report")
    kiwoom_rest_quote_mocked_response_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_quote_mocked_response_report.add_argument("--output-file", type=Path)
    kiwoom_rest_canonical_quote_report = subparsers.add_parser("kiwoom-rest-canonical-quote-report")
    kiwoom_rest_canonical_quote_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_canonical_quote_report.add_argument("--output-file", type=Path)
    kiwoom_rest_canonical_orderbook_report = subparsers.add_parser("kiwoom-rest-canonical-orderbook-report")
    kiwoom_rest_canonical_orderbook_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_canonical_orderbook_report.add_argument("--output-file", type=Path)
    kiwoom_rest_liquidity_hint_report = subparsers.add_parser("kiwoom-rest-liquidity-hint-report")
    kiwoom_rest_liquidity_hint_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_liquidity_hint_report.add_argument("--output-file", type=Path)
    kiwoom_rest_basic_info_report = subparsers.add_parser("kiwoom-rest-basic-info-report")
    kiwoom_rest_basic_info_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_basic_info_report.add_argument("--output-file", type=Path)
    kiwoom_rest_quote_continuation_report = subparsers.add_parser("kiwoom-rest-quote-continuation-report")
    kiwoom_rest_quote_continuation_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_quote_continuation_report.add_argument("--output-file", type=Path)
    kiwoom_rest_quote_readonly_safety_report = subparsers.add_parser("kiwoom-rest-quote-readonly-safety-report")
    kiwoom_rest_quote_readonly_safety_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_quote_readonly_safety_report.add_argument("--output-file", type=Path)
    kiwoom_rest_quote_v7_integration_report = subparsers.add_parser("kiwoom-rest-quote-v7-integration-report")
    kiwoom_rest_quote_v7_integration_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_quote_v7_integration_report.add_argument("--output-file", type=Path)
    ...
    kiwoom_rest_sector_adapter_check = subparsers.add_parser("kiwoom-rest-sector-adapter-check")
    kiwoom_rest_sector_adapter_check.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_sector_adapter_check.add_argument("--output-file", type=Path)
    kiwoom_rest_sector_request_report = subparsers.add_parser("kiwoom-rest-sector-request-report")
    kiwoom_rest_sector_request_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_sector_request_report.add_argument("--output-file", type=Path)
    kiwoom_rest_sector_mocked_response_report = subparsers.add_parser("kiwoom-rest-sector-mocked-response-report")
    kiwoom_rest_sector_mocked_response_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_sector_mocked_response_report.add_argument("--output-file", type=Path)
    kiwoom_rest_canonical_theme_leadership_report = subparsers.add_parser("kiwoom-rest-canonical-theme-leadership-report")
    kiwoom_rest_canonical_theme_leadership_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_canonical_theme_leadership_report.add_argument("--output-file", type=Path)
    kiwoom_rest_canonical_theme_membership_report = subparsers.add_parser("kiwoom-rest-canonical-theme-membership-report")
    kiwoom_rest_canonical_theme_membership_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_canonical_theme_membership_report.add_argument("--output-file", type=Path)
    kiwoom_rest_canonical_etf_trend_report = subparsers.add_parser("kiwoom-rest-canonical-etf-trend-report")
    kiwoom_rest_canonical_etf_trend_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_canonical_etf_trend_report.add_argument("--output-file", type=Path)
    kiwoom_rest_sector_etf_capability_matrix_report = subparsers.add_parser("kiwoom-rest-sector-etf-capability-matrix-report")
    kiwoom_rest_sector_etf_capability_matrix_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_sector_etf_capability_matrix_report.add_argument("--output-file", type=Path)
    kiwoom_rest_sector_continuation_report = subparsers.add_parser("kiwoom-rest-sector-continuation-report")
    kiwoom_rest_sector_continuation_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_sector_continuation_report.add_argument("--output-file", type=Path)
    kiwoom_rest_sector_readonly_safety_report = subparsers.add_parser("kiwoom-rest-sector-readonly-safety-report")
    kiwoom_rest_sector_readonly_safety_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_sector_readonly_safety_report.add_argument("--output-file", type=Path)
    kiwoom_rest_sector_v7_integration_report = subparsers.add_parser("kiwoom-rest-sector-v7-integration-report")
    kiwoom_rest_sector_v7_integration_report.add_argument("--fixture-file", type=Path, required=True)
    kiwoom_rest_sector_v7_integration_report.add_argument("--output-file", type=Path)
...
def run_command(args: argparse.Namespace) -> dict[str, object]:
    if args.command == "legacy-evaluate":
    ...
    if args.command == "kiwoom-rest-chart-gap-report":
        try:
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-rank-adapter-check":
        try:
            result = _run_kiwoom_rest_readonly_rank(args.fixture_file).summary_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "readiness": result.readiness.value}
            return {"readiness": result.readiness.value, **result.model_dump(mode="json")}
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-rank-request-report":
        try:
            result = _run_kiwoom_rest_readonly_rank(args.fixture_file).request_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-rank-mocked-response-report":
        try:
            result = _run_kiwoom_rest_readonly_rank(args.fixture_file).mocked_response_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-canonical-rank-report":
        try:
            result = _run_kiwoom_rest_readonly_rank(args.fixture_file).canonical_rank_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-canonical-outlier-report":
        try:
            result = _run_kiwoom_rest_readonly_rank(args.fixture_file).canonical_outlier_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-rank-continuation-report":
        try:
            result = _run_kiwoom_rest_readonly_rank(args.fixture_file).continuation_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-rank-readonly-safety-report":
        try:
            result = _run_kiwoom_rest_readonly_rank(args.fixture_file).safety_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "safety_report_id": result.safety_report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-rank-v7-integration-report":
        try:
            result = _run_kiwoom_rest_readonly_rank(args.fixture_file).v7_integration_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-rank-gap-report":
        try:
            result = _run_kiwoom_rest_readonly_rank(args.fixture_file).gap_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "gap_report_id": result.gap_report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-quote-adapter-check":
        try:
            result = _run_kiwoom_rest_readonly_quote(args.fixture_file).summary_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "readiness": result.readiness.value}
            return {"readiness": result.readiness.value, **result.model_dump(mode="json")}
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-quote-request-report":
        try:
            result = _run_kiwoom_rest_readonly_quote(args.fixture_file).request_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-quote-mocked-response-report":
        try:
            result = _run_kiwoom_rest_readonly_quote(args.fixture_file).mocked_response_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-canonical-quote-report":
        try:
            result = _run_kiwoom_rest_readonly_quote(args.fixture_file).canonical_quote_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-canonical-orderbook-report":
        try:
            result = _run_kiwoom_rest_readonly_quote(args.fixture_file).canonical_orderbook_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-liquidity-hint-report":
        try:
            result = _run_kiwoom_rest_readonly_quote(args.fixture_file).liquidity_hint_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-basic-info-report":
        try:
            result = _run_kiwoom_rest_readonly_quote(args.fixture_file).basic_info_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-quote-continuation-report":
        try:
            result = _run_kiwoom_rest_readonly_quote(args.fixture_file).continuation_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-quote-readonly-safety-report":
        try:
            result = _run_kiwoom_rest_readonly_quote(args.fixture_file).safety_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "safety_report_id": result.safety_report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-quote-v7-integration-report":
        try:
            result = _run_kiwoom_rest_readonly_quote(args.fixture_file).v7_integration_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-quote-gap-report":
        try:
            result = _run_kiwoom_rest_readonly_quote(args.fixture_file).gap_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "gap_report_id": result.gap_report_id}
            return result.model_dump(mode="json")
    ...
    if args.command == "kiwoom-rest-flow-gap-report":
        try:
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-sector-adapter-check":
        try:
            result = _run_kiwoom_rest_readonly_sector(args.fixture_file).summary_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "readiness": result.readiness.value}
            return {"readiness": result.readiness.value, **result.model_dump(mode="json")}
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-sector-request-report":
        try:
            result = _run_kiwoom_rest_readonly_sector(args.fixture_file).request_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-sector-mocked-response-report":
        try:
            result = _run_kiwoom_rest_readonly_sector(args.fixture_file).mocked_response_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-canonical-theme-leadership-report":
        try:
            result = _run_kiwoom_rest_readonly_sector(args.fixture_file).canonical_theme_leadership_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-canonical-theme-membership-report":
        try:
            result = _run_kiwoom_rest_readonly_sector(args.fixture_file).canonical_theme_membership_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-canonical-etf-trend-report":
        try:
            result = _run_kiwoom_rest_readonly_sector(args.fixture_file).canonical_etf_trend_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-sector-etf-capability-matrix-report":
        try:
            result = _run_kiwoom_rest_readonly_sector(args.fixture_file).sector_etf_capability_matrix_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-sector-continuation-report":
        try:
            result = _run_kiwoom_rest_readonly_sector(args.fixture_file).continuation_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-sector-readonly-safety-report":
        try:
            result = _run_kiwoom_rest_readonly_sector(args.fixture_file).safety_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "safety_report_id": result.safety_report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-sector-v7-integration-report":
        try:
            result = _run_kiwoom_rest_readonly_sector(args.fixture_file).v7_integration_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "report_id": result.report_id}
            return result.model_dump(mode="json")
            ...
            return {"status": "FAILED", "errors": [str(exc)]}
    if args.command == "kiwoom-rest-sector-gap-report":
        try:
            result = _run_kiwoom_rest_readonly_sector(args.fixture_file).gap_report
            if args.output_file:
                args.output_file.write_text(result.model_dump_json(indent=2), encoding="utf-8")
                return {"status": "COMPLETED", "output_file": str(args.output_file), "gap_report_id": result.gap_report_id}
            return result.model_dump(mode="json")
...
def _load_kiwoom_rest_readonly_rank_fixture_or_raise(fixture_file: Path):
    return load_kiwoom_rest_readonly_rank_fixture(fixture_file)


def _run_kiwoom_rest_readonly_rank(fixture_file: Path):
    fixture = _load_kiwoom_rest_readonly_rank_fixture_or_raise(fixture_file)
    return build_kiwoom_rest_readonly_rank_adapter(fixture)


def _load_kiwoom_rest_readonly_quote_fixture_or_raise(fixture_file: Path):
    return load_kiwoom_rest_readonly_quote_fixture(fixture_file)
...
def _load_kiwoom_rest_readonly_sector_fixture_or_raise(fixture_file: Path):
    return load_kiwoom_rest_readonly_sector_fixture(fixture_file)
