...
_UNSAFE_PATTERNS = (
    ...
    re.compile(r"api[_ -]?key|secret|credential|password|token(?!_ref_only)", re.IGNORECASE),
    re.compile(r"os\.environ|env[_ -]?var|dotenv|credential file", re.IGNORECASE),
    re.compile(r"account(_id|_number)?|order(_id)?|execution|broker[_ -]?order", re.IGNORECASE),
    re.compile(r"kt\d{5}|ka00001|credit|gold order|websocket|network", re.IGNORECASE),
    re.compile(r"live|prod|autonomous", re.IGNORECASE),
    re.compile(r"gemini|cloud[_ -]?llm|ollama|llama|transformers", re.IGNORECASE),
)


def validate_kiwoom_rest_chart_metadata_safety(value, *, context: str) -> None:
