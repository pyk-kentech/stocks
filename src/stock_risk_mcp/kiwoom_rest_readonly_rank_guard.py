...
_UNSAFE_PATTERNS = (
    ...
    re.compile(r"kt\d{5}|ka00001|credit order|gold order|websocket|network", re.IGNORECASE),
    re.compile(r"live|prod|autonomous", re.IGNORECASE),
    re.compile(r"gemini|cloud[_ -]?llm|ollama|llama|transformers", re.IGNORECASE),
)


def validate_kiwoom_rest_rank_metadata_safety(value, *, context: str) -> None:
