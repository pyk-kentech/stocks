...
def test_domestic_realtime_core_has_no_forbidden_imports_or_artifact_creation():
    root = Path(__file__).resolve().parents[1] / "src" / "stock_risk_mcp"
    files = (
        "domestic_realtime_models.py",
        "domestic_realtime_fixture.py",
        "domestic_realtime_engine.py",
        "domestic_realtime_service.py",
    )
    forbidden = (
        "database",
        "repository",
