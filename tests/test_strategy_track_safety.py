...
def test_strategy_track_core_has_no_forbidden_imports_or_artifact_creation():
    root = Path(__file__).resolve().parents[1] / "src" / "stock_risk_mcp"
    files = (
        "strategy_track_models.py",
        "strategy_track_fixture.py",
        "strategy_track_engine.py",
        "strategy_track_service.py",
    )
